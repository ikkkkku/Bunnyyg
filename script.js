...
只能输出纯净 JSON 对象，严禁使用 \`\`\`json 等代码块，严禁包含任何思考过程、旁白或解释！`;
    ...
    function updatePreviewStyle(containerId, styleObj) {
        ...
        // 生成针对预览容器的CSS
        let css = generateChatCss(styleObj, true);
        // 加上容器前缀限制作用域
        css = css.replace(/(?:\r\n|\r|\n)/g, ' ').split('}').map(rule => {
            if (!rule.trim()) return '';
            return `#${containerId} ` + rule.trim() + '}';
        }).join(' ');
        styleTag.innerHTML = css;
    }
    // --- 全局美化逻辑 ---
...
document.getElementById('personal-style-back').addEventListener('click', async () => {
    ...
    // --- 新增：生成聊天美化 CSS 的通用函数 ---
    function generateChatCss(styleObj, isPreview = false) {
        if (!styleObj) return '';
        let css = '';
        // 如果是预览，限定前缀为预览容器，否则为全局聊天内容
        const prefix = isPreview ? '' : '#chat-detail-content ';
        const aiBubble = isPreview ? '.ai-bubble' : '.ai-bubble';
        const userBubble = isPreview ? '.user-bubble' : '.user-bubble';
        const avatar = isPreview ? '.vp-avatar' : '.msg-avatar';
        const bubble = isPreview ? '.vp-bubble' : '.msg-bubble';
        if (styleObj.color === 'blue') {
            css += `${prefix}${userBubble} { background: #e6f7ff !important; color: #0050b3 !important; } ${prefix}${aiBubble} { background: #ffffff !important; border: 1px solid #bae0ff !important; color: #0050b3 !important; } `;
        } else if (styleObj.color === 'green') {
            css += `${prefix}${userBubble} { background: #f6ffed !important; color: #389e0d !important; } ${prefix}${aiBubble} { background: #ffffff !important; border: 1px solid #b7eb8f !important; color: #389e0d !important; } `;
        } else if (styleObj.color === 'purple') {
            css += `${prefix}${userBubble} { background: #f9f0ff !important; color: #531dab !important; } ${prefix}${aiBubble} { background: #ffffff !important; border: 1px solid #d3adf7 !important; color: #531dab !important; } `;
        } else if (styleObj.color === 'dark') {
            css += `${prefix}${userBubble} { background: #2b2b2b !important; color: #e8e8e8 !important; } ${prefix}${aiBubble} { background: #1f1f1f !important; border: 1px solid #434343 !important; color: #e8e8e8 !important; } `;
        }
        if (styleObj.size && styleObj.size != 6) {
            css += `${prefix}${bubble} { padding: ${styleObj.size}px 12px !important; } `;
        }
        if (styleObj.radius && styleObj.radius != 12) {
            css += `${prefix}${bubble} { border-radius: ${styleObj.radius}px !important; } `;
            css += `${prefix}${aiBubble} { border-top-left-radius: 2px !important; } ${prefix}${userBubble} { border-top-right-radius: 2px !important; } `;
        }
        // 头像框处理
        if (styleObj.avatar === 'circle') {
            css += `${prefix}${avatar} { border: none !important; box-shadow: none !important; } `;
        } else if (styleObj.avatar === 'square') {
            css += `${prefix}${avatar} { border-radius: 12px !important; } `;
        } else if (styleObj.avatar === 'shadow') {
            css += `${prefix}${avatar} { box-shadow: 0 6px 16px rgba(0,0,0,0.4) !important; border: 1px solid rgba(255,255,255,0.5) !important; } `;
        } else if (styleObj.avatar === 'neon') {
            css += `${prefix}${avatar} { border: 2px solid #fff !important; box-shadow: 0 0 8px var(--theme-pink), 0 0 16px var(--theme-pink-dark) !important; } `;
            ...
            css += `${prefix}${avatar} { border: none !important; box-shadow: none !important; } `;
            if (isPreview) {
                css += `${prefix}.vp-avatar { position: relative; } `;
                css += `${prefix}.vp-avatar::after { content: ''; position: absolute; top: 0; left: 50%; width: 40px; height: 40px; background-image: url('${styleObj.frameImg}'); background-size: contain; background-position: center; background-repeat: no-repeat; pointer-events: none; transform: translate(calc(-50% + ${x}px), ${y}px) scale(${scale}); z-index: 10; } `;
            } else {
                css += `${prefix}.msg-avatar-col { position: relative; } `;
                css += `${prefix}.msg-avatar-col::after { content: ''; position: absolute; top: 0; left: 50%; width: 36px; height: 36px; background-image: url('${styleObj.frameImg}'); background-size: contain; background-position: center; background-repeat: no-repeat; pointer-events: none; transform: translate(calc(-50% + ${x}px), ${y}px) scale(${scale}); z-index: 10; } `;
            }
        }
        if (styleObj.css) {
            css += `\n/* 自定义CSS */\n${styleObj.css}\n`;
    ...
    let globalChatStyleData = null; // 新增：显式声明全局变量，防止未初始化报错
    function applyGlobalChatStyle(styleObj) {
        ...
        styleTag.innerHTML = generateChatCss(styleObj, false);
    }
    ...
    document.getElementById('btn-open-personal-style').addEventListener('click', () => {
        if (!currentActiveChat) return;
        const pStyle = currentActiveChat.personalStyle || globalChatStyleData || {};
        ...
        document.getElementById('ps-vp-user-avatar').style.backgroundImage = `url('${uAvatar}')`;
        document.getElementById('ps-vp-user-avatar').style.backgroundSize = 'cover';
        document.getElementById('ps-vp-ai-avatar').style.backgroundImage = `url('${aAvatar}')`;
        document.getElementById('ps-vp-ai-avatar').style.backgroundSize = 'cover';
        triggerPersonalPreview();
...
- 严禁输出任何 Markdown 标记（如 \`\`\`json），严格输出纯 JSON 对象。格式如下：
{
  ${formatPrompt}
    ...
    async function getChatStylePresets() {
        ...
        // 自动创建“美化模板一”
        if (presets.length === 0 || !presets.find(p => p.id === 'default_template_1')) {
            presets.push({
                ...
                style: {
                    ...
                    frameY: 0,
                    css: `/* 1. 双方圆形头像 */
.msg-avatar { border-radius: 50% !important; }
/* 2. 气泡极小尖角在上方，适中圆角，无外阴影。对方 #F8F6F7，我方 #FEF2FB */
.ai-bubble { border-radius: 2px 14px 14px 14px !important; background: #F8F6F7 !important; color: #5a4b50 !important; box-shadow: none !important; }
.user-bubble { border-radius: 14px 2px 14px 14px !important; background: #FEF2FB !important; color: #5a4b50 !important; box-shadow: none !important; }
/* 3. 每轮只有首条消息显示头像，其余隐藏头像保留占位 */
.msg-row.ai + .msg-row.ai .msg-avatar { opacity: 0 !important; pointer-events: none !important; }
.msg-row.user + .msg-row.user .msg-avatar { opacity: 0 !important; pointer-events: none !important; }
/* 4. 紧贴上一条气泡，间距只有 5px (默认gap 12px - 7px = 5px) */
.msg-row.ai + .msg-row.ai { margin-top: -7px !important; }
.msg-row.user + .msg-row.user { margin-top: -7px !important; }
/* 5. 时间戳只显示在最后一条气泡底部下方，格式为「时：分 Read」 */
.msg-time { display: none !important; }
.msg-row.ai:not(:has(+ .msg-row.ai)), .msg-row.user:not(:has(+ .msg-row.user)) { margin-bottom: 16px !important; position: relative; }
.msg-row.ai:not(:has(+ .msg-row.ai)) .msg-time, .msg-row.user:not(:has(+ .msg-row.user)) .msg-time { display: block !important; position: absolute; bottom: -16px; font-size: 10px !important; color: #cbaeb4 !important; transform: scale(1) !important; white-space: nowrap; }
.msg-time::after { content: " Read"; }
/* 6. 对方时间戳左对齐，我方时间戳下方左对齐（贴近气泡右侧以确保对齐视觉差） */
.msg-row.ai:not(:has(+ .msg-row.ai)) .msg-time { left: 46px; text-align: left; }
.msg-row.user:not(:has(+ .msg-row.user)) .msg-time { right: 46px; text-align: left; }`
                }
            });
